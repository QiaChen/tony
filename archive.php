...
<div id="header_info" class="index-top">
    ...
    <div class="index-cates">
        <li datacates="cate" class="cat-item cat-item-4 cat-real"  style="display:none"  v-if="!loading_des"  >  <a href="/" >主页</a> </li>
        <li datacates="cate" v-bind:class="[ '<?php echo $id?>' == de.id? 'on' : '', 'cat-item','cat-item-4','cat-real']"  style="display:none" v-for="de in des" v-if="de.count !== 0"  >  <a :href="de.link" :title="de.description" v-html="de.name"></a>
        </li>
...
<script>
    $(document).ready(function() { //避免爆代码
        ...
        $(".index-cates [datacates]").attr('style', 'display:inline-block');
        /* 展现内容(避免爆代码) */
